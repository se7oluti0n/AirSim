...
def main():
    ...
    while running:
        ...
        # Apply PTZ updates
        try:
            # *** FIX for simSetCameraOrientation not found error ***
            # Get the vehicle's current pose to ensure the camera stays with it.
            vehicle_pose = client.simGetVehiclePose(vehicle_name=VEHICLE_NAME)
            
            # Create the desired camera orientation
            orientation_quat = airsim.to_quaternion(math.radians(tilt), 0, math.radians(pan))
            
            # Combine the vehicle's position with the new camera orientation
            new_camera_pose = airsim.Pose(vehicle_pose.position, orientation_quat)
            
            # Set the new camera pose
            client.simSetCameraPose(CAMERA_NAME, new_camera_pose, vehicle_name=VEHICLE_NAME)
        ...
        # Get image from AirSim AFTER updating the camera pose
